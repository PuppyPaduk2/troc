...
const proxyHandler: RequestEventHandler = async (
  event,
  { response, request }
) => {
  const send = attachResponse(response);

  const redirectResult = await redirectNpmRequest(event, request);
...
const localHandler: RequestEventHandler = async (
  event,
  { response, request }
) => {
  const send = attachResponse(response);

  const pkgInfo = await getIncomingMessageJson<NpmPackageInfoPublish>(request);
